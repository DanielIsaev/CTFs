...
+ Other RPC related port in 593, 49159.

Based on the service finger printing it seems that the host OS might be a 2012 Windows server R2. 
From the SMB scanning we can see that we have an FQDN of `rell.htb.local`, could this be a DC? 
also based on the SMB scanning we can see that message signing is enabled and required. 

What stands out the most are the FTP, SMTP and SMB servers so let's take a look at thoes. 

## FTP Port 21

Lets try to log in with an anonymous account `anonymous : anonymous` :
...
We have a couple of Microsoft word documents and a text file. Let's copy all of them over to our machine to examine. We can do this with one liner:
...
The note also says that if we send an `rtf` file via email they will be converted and saved here. I presume that by "here" they ment the FTP server, therefore i'm guessing that the two `Word` documents here, are examples of `rtf` files that have been converted to `Word` documents.
...
This is a Python 2 exploit for a known vulnerability, [CVE-2017-0199](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2017-0199). This script can both generate an `rtf` file with an embedded link inside it. 
This file has to be delivered to a victim user which needs to click the embedded link. When they do, the `rtf` parser will fetch the file and execute it. So we need to generate the malicious `rtf` and host a reverse shell as the embedded link.


There were also two `docx` files we got from the FTP. We can convert these file to text file on linux with `docx2txt`, i.e: `docx2text <in_file.docx> <out_file.txt>`

+ 'Windows Event Forwarding.docx'

At first i wasn't familier with `Windows Event Forwarding` so i searched online, and found [this](https://learn.microsoft.com/en-us/windows/security/threat-protection/use-windows-event-forwarding-to-assist-in-intrusion-detection) artical from Microsoft, as well as [this](https://www.rapid7.com/blog/post/2018/12/18/windows-event-forwarding-the-best-thing-youve-never-heard-of/) from Rapid7. 

In short, the `Windows Event Forwarding` system or `WEF` for short, is a way to get all event logs from endpoint Windows computers (endpoints) onto a centralized server collector, A.K.A Windows Event Collector (WEC). This is achived using the `WinRM` protocol for remote managment of Windows systems. 
...
I took a step back, and thought, "maybe there is hidden information in the `docx` files?".
So I transferred the files over to a Windows machine and inspected the documents.
...
I did a bit of research as to why exactly the vulnerability happens, I found [this](https://www.mdsec.co.uk/2017/04/exploiting-cve-2017-0199-hta-handler-vulnerability/#:~:text=FireEye%20recently%20documented%20attacks%20of,the%20April%202017%20Microsoft%20Update.) artical that gave a nice little summary. 
...
The `-M` specifys we are running in exploit mode, the `-e` is the URL for the file to execute, and the `-l` is the local path to the file if its hosted locally.
...
We can send an email with an attachment easily using `swaks`:
...
But we didnt connect on the reverse shell, why? at first I tried to use different payloads, staged, non-staged, x86 payloads, metasploit, etc, nothing worked. 

So I took a look at the exploit source code, I started by saerching for the `Received GET method` string:
...
If you pay attention youl'l find that there are two definitions of the `server_thread()` function, once after the difinition of the `exploitation_rtf()` function, and the other is defined after the difinition of the `exploitation_ppsx()` fucntion. 

Each `server_thread()` definition is similar in stucture to the other, but has slight varitions in it's responses and and command injection. I believe that the author defined each variation to be used by a different exploitaion function, but this is not the case. When the Python compiler transforms the source code into machine code, the last definition of the `server_thread()` will overwrite the first one. 

We can see each `server_thread()` function being invoked at the end of each exploitation function with
`thread.start_new_thread(server_thread, (conn, client_addr))`. What we can do is change the name of the `server_thread()` function to something unique, and call each function by it's unique name:

![mod-19]()

Now when we execute, we can see a different result:
...
So at first, the target fetches `sale.doc`, to which our server replies with HTML code that has a javascript payload to run powershell and fetch the real reverse shell `exe` payload from our target and execeute it. We can see this worked, but we still didnt get the shell. What could be the issue now?

I kept looking at the source code and I found an intersting option:

![hta-22]()

We know that `exe` files, along with other forms of executable formats are monitored by Applocker, perhaps its the one blocking the execution of our `exe` ? So instead we can try to use an `hta` paylaod. 
...
Then, we need to setup the toolkit server as such:
...
I start the escalation stage by hunting for quick wins. At first i look for my user's privleges with `whoami /priv`, but no interesting privileges are set. 
...
But this comes out empty as well. So I decided to list all the users on this machine:
...
And here we see something interesting, we have the `Kerberos Ticket Granting Ticket` service account, 
this machine is probably a Domain Controller. We can verify this by running `nltest /dclist:htb.local`:
...
So instead I decided to look around the system. I started with our user's home folder, in the desktop 
folder we see an interesting file:
...
To do this we need an active powershell session, luckily we have SSH enabled on this machine, and we can use the credentials we found erlier to get access. Once we are connected via the SSH we need to start a powershell session with `powershell -exec bypass`. 

Here are the steps to reproduce the password. 
...
First, I went to Tom's desktop folder where I found something interesting:

![ad-audit-32]()

Unusual, if we list the contents of this directory we see the following:
...
This is referring to a BloodHound security audit they ran againts the domain. They mention the shortest path query coming out empty for them. The shortest path to domain admin is a custom query we can run in BloodHound to get possible escalation paths. 
...
So instead I decided to run the local scripts. This works, and they produce `csv` files with varoius information. But when I try to load these files into the BloodHound Visualizer I get the following error:
...
If the difference in versions between the collector and the visualizer are too big we might encounter this `Incompatible Collector` issue. The same post suggests that the output from the `SharpHound.exe` collctor should be acceepted by newer versions of the Visualizer, but we have an issue on our target with the AppLocker blocking its execution. 

I've tried to bypass the Applocker, I found [this](https://juggernaut-sec.com/applocker-bypass/) post, as well as [this](https://github.com/api0cradle/UltimateAppLockerByPassList) page, that detial how we can try to bypass the Applocker. I've tested a few different techinques but none of them worked for me. And nothing really guarantees that the ouptut from the exe collector will actually be accepted by the visualizer. 

So instead I decided to install BloodHound V3.0.5 and neo4j V4.1 on a different Ubuntu machine I have, and run the collector from that version to collect the data from the target.
...
We have `WriteOwner` access over Claire's account. If we click on the Help button, we get a list of different attacks we can perfrom, utiliazing this access. What I would like to do is reset Claire's password using the `Set-DomainUserPassword` function of `PowerView.ps1` and perfrom an account take over.
...
I tried to look for escalation path's from the `backup_admins` group to the domain admin group in BloodHound, but couldn't find any, but at any case, let's perform the attack and add Claire to the group:
