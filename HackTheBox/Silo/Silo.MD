...
We get quite a lot of open ports. I'm guessing a lot of them are RPC ports, while other ports are shwon as filtered. Let's run a service and version detection scan on the open ports with:
...
But, using `crackmapexec smb silo.htb --users` I did managed to get the target OS version and architecture:
...
As for step one, we got the version from the namp scan (11.2.0.2.0), I searched for known exploits for this version but I didnt find any. 

In step two we want to find valid SIDs on the listener. The SID (Service Identifier) is essentially the database name, depending on the install you may have one or more default SIDs, or even a totally custom dba defined SID. To enumerate SID we can use metasploit modules like `auxiliary/scanner/oracle/sid_enum` and  `auxiliary/admin/oracle/tnscmd` but I rather avoid metasploit. 

Instead we can use a custom tool called [tnscmd10g](https://www.kali.org/tools/tnscmd10g/), `apt install tnscmd10g`. 
...
Unfortenataly this resulted in an `12618` error. Based on the [following resources](https://docs.oracle.com/database/121/ERRMG/TNS-00000.htm#ERRMG-GUID-D723D931-ECBA-4FA4-BF1B-1F4FE2EEBAD7), this means that the TNS version is incompatible. In this case we can use the `-10G` switch:
...
But this didnt help in my case. So I decided to use [odat](https://github.com/quentinhardy/odat) instead. Odat is a powerful tool for hacking Oracle databases. It can enumerate SIDs, find valid credentials, and exploit different attacks like TNS posining and RCE. The tool can be installed with `apt install odat`
...
We can see we dont ahve the `DBA` role, but we have other roles we can leverge to get our user the admin roles. For example, we can give a database user admin roles using the CREATE/EXECUTE ANY PROCEDURE privileges:
...
Now we can use the `--sysdba` switch to perform the operation with db-admin privileges. This time I decided to change the command syntax because I was having issues with the previous command and I wanted to verify the success of the RCE, so I chose to ping my attack machine while capturing the network traffic using:
...
This operation finsihed without any errors:

![ran-18]()


And in the `tcpdump` shell I got a ping:
...
Unfortenatly, this shell is a bit limited. For example running `whoami` or `hostname` doesnt work on the target. But this is enough to get the user flag:

![user-22]()

## Privilege Escalation

After landing on the machine I start hunting for quick wins like `whoami /priv` for a potato attack, `cmdkey /list` for stored creds, looking for creds in the registry but none of them worked for me. 
...
Something is off with the first character of the password, probably encoding issues. There is a trick we can use in these situations. We can encode the fiel into bytes, then base64 encode the bytes, and then decode the base64 into utf-8. 
...
Now we can see the password has changed, the first character is actually `£`. Now if we try to open the DropBox share:
...
The file is a zip arachive so I use `unzip` to extract it. Inside I find another zip archive named `SILO-20180105-221806.zip`, I use `uznip` once again and now I have the dump file:
...
After youv'e installed `volatility`, we can start analyzing tyhe core dump. First we need to use the `kdbgscan` module so we can find a working profile that fits the core dump:
...
The SAM file is what we want. Now that we have the system and SAM registry offests we can extract its contents using the `hashdump` module:
