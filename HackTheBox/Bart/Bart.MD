...
One of these employee's user account could be a potential an entry point. Lets keep looking. We can check the backend tech 
with `Wappalyzer`: 
...
.txt -u http://monitor.bart.htb/FUZZ -fc 500 -e .php,.apsx,.xml,.txt
```
...
At this point i spent about 4 hours on this box, and i had no leads on how to proceed. So decided to 
...
we get the server error. We can conclude from this that the `register.php` is present on the server. 
