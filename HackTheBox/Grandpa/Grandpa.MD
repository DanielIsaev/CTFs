...
While I was performing these tests, I left `ffuf` running in the background. Near the end of the scan it found an interesting HTML file:

![postinfo-08.5]()

If we open this file we get the folloinwg:
...
This reveals more directories. I ran `davtest` but unfortenatly nothing worked. I think it's safe to conclude that we cant manually exploit the `WebDAV` itself. Let's go for the buffer-overfow expliot.
