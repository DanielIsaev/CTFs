...
Another point where this is visible is with `orestis`'s signature, we know from the previous channel that `orestis` always signs he's message with `Orestis - Hacking for fun and profit`. But the cipher text for this string is different in every message, which means that the key is always different.
...
This is looks lik a python script, but it's really [SageMath](https://en.wikipedia.org/wiki/SageMath) script, which is an open source mathematical programming language build on top of Python, so the syntax is similar. The question is what is the script doing? 
...
I did a bit of research online and I found [this perfect StackOverflow](https://stackoverflow.com/questions/49856115/inverse-of-a-powa-b-n-function-in-python-decryption-code?noredirect=1&lq=1) thread, which fites our problem like a glove. 
